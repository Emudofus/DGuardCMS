...
function button_to($name, $internal_uri, $options = array())
{
  ...
  if (isset($html_options['popup']) || isset($html_options['confirm']))
  {
...
function _convert_options_to_javascript($html_options, $url = 'this.href')
{
  ...
  {
    $html_options['onclick'] = $onclick.'if ('._confirm_javascript_function($confirm).') { '._method_javascript_function($method, $url).' };return false;';
  }
  else if ($confirm)
  {
    if ($onclick)
...
function _method_javascript_function($method, $url = 'this.href')
{
  $function = "var f = document.createElement('form'); f.style.display = 'none'; this.parentNode.appendChild(f); f.method = 'post'; f.action = {$url};";
